...
import InfographicMenu from '../components/InfographicMenu';
...
const Domaines = () => {
  ...
  const columns = [
    { header: 'Code', accessor: 'code' as keyof Domaine },
    { header: 'Name', accessor: 'name' as keyof Domaine },
    { header: 'Nature', accessor: 'nature' as keyof Domaine },
    {
      header: 'Actions',
      accessor: (domaine: Domaine) => (
        <div className="flex gap-2">
          <Button onClick={() => openEditModal(domaine)} variant="secondary">
            Edit
          </Button>
          <Button onClick={() => handleDelete(domaine._id)} variant="danger">
            Delete
          </Button>
          <Button onClick={() => window.location.href = `/dashboard/domaines/${domaine._id}`}>View</Button>
        </div>
      )
    }
  ];
      ...
      {loading ? (
        ...
        <div className="flex flex-wrap gap-8 justify-center items-center">
          {domaines.map((domaine) => (
            <div key={domaine._id} className="flex flex-col items-center">
              <InfographicMenu
                code={domaine.code}
                name={domaine.name}
              ...
              />
              <span className="mt-2 text-sm text-gray-500 max-w-xs text-center">{domaine.description || <span className="text-gray-400">Aucune description</span>}</span>
              <span className="mt-1 text-xs text-gray-400">{domaine.nature}</span>
            </div>
          ))}
        </div>
      )}
